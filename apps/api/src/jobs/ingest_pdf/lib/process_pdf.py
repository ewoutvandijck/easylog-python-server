...
async def process_pdf(
    ...
    await supabase.storage.from_(target_bucket).upload(
        full_pdf_path,
        file_data,
        {
            "upsert": "true",
    ...
    pdf_file_object = await prisma.objects.find_first_or_raise(
        where={"name": full_pdf_path, "bucket_id": target_bucket}
    )

...
- Vat grote datasets bondig samen
        ...
        full_buffered_content = []

        async for content in self.handle_stream(stream, tools):
            # Als het een afbeelding bevat, markeer dat we aan het bufferen zijn
            if hasattr(content, "type") and content.type == "image":
                has_image_content = True
                self.logger.info("Detected image content, buffering complete response")

            # Voeg elke chunk toe aan de volledige buffer
            full_buffered_content.append(content)

        # Als er afbeeldingen in de content zitten, stuur alles in één keer om corruptie te voorkomen
        if has_image_content:
            self.logger.info(f"Sending complete buffered response with images ({len(full_buffered_content)} chunks)")
            # Stuur alles in één keer
            for content_chunk in full_buffered_content:
                yield content_chunk
        else:
            # Geen afbeeldingen, we kunnen het direct streamen vanuit de buffer
            self.logger.info("No images detected, sending response chunks")
            for content_chunk in full_buffered_content:
                if self.config.debug_mode:
                    self.logger.debug(f"Streaming content chunk: {str(content_chunk)[:100]}...")
                yield content_chunk
